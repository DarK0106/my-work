...
public class AiService {
	...
	// AI가 DB 구조를 알아야 하니까 가져옴
	@Value("Classpath:/script.sql")
	private Resource sqlResource;

	// 사용자가 질문한 프롬프트: text
	public SqlResponse execute(String text) throws IOException {
		...
		String template = """
				    ...
				    9. 질문이 모호하거나 DDL만으로 답할 수 없으면 "지원하지 않음"이라고만 출력하세요. 환각 없이 대답하세요.
		...
		String schema = sqlResource.getContentAsString(Charset.defaultCharset());

		String query = chatClient.prompt().user(sp -> sp.text(template).param("question", text).param("ddl", schema))
				.call().content();

		//		System.out.println(query);

		// SQL 실행 -> 결과(이건 개발자가 해야 함, LLM의 역할은 딱 윗부분 까지임)
		if (query.toLowerCase().startsWith("select")) {

			// List: 전체 결과(여러 행), Map: 한 행, String: 컬럼명, Object: 해당 컬럼의 값
			// query, Jdbctemplate.queryForList(query) 이 둘을 한곳에 담아야 해서
			// DTO(SqlResponse 클래스)를 만든 것
