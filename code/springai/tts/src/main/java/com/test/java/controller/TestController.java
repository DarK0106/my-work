...
public class TestController {
	...
	@PostMapping(value = "/upload", produces = "audio/mpeg")
	@ResponseBody
	public StreamingResponseBody upload(@RequestParam("attach") MultipartFile attach) throws IOException {
	    ...
	    // 번역? 텍스트를 음성으로 바꾼다는것(대충 그런 의미)
		// AI 에게 문장을 읽어달라고 요청 (실시간 스트리밍)
		Flux<byte[]> responseStream = aiService.streamAudio(content);

		// 스트리밍 응답 반환 (브라우저로 오디오 청크 흘려보내기)
		return outputStream -> responseStream.toStream()
				.forEach(bytes -> writeToOutput(outputStream, bytes));
	}
